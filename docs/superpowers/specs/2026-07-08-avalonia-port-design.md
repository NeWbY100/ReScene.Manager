...
4. **User settings folder — DECIDED (2026-07-08): fresh start, no migration.** `JsonFileStore.cs:17-19` persists AppSettings, RecentFiles, and WindowState to `%LOCALAPPDATA%\ReScene.NET`. ReScene Manager will write to a **new `ReScene.Manager` folder** (rename the folder literal in `JsonFileStore`); existing users start with default settings. No migration routine. Accepted regression: current users lose saved settings/recent-files/window placement on first run of the renamed app. (Property casing is already safe via `PropertyNameCaseInsensitive`, so this is purely a folder-name change.)
