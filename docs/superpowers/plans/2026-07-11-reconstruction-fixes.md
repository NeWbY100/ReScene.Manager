# RAR Reconstruction Correctness Fixes — Implementation Plan (rev. 7, FROZEN — ready to execute)
...
**Tech Stack:** .NET 10 · CommunityToolkit.Mvvm 8.4 · xUnit · `Microsoft.Extensions.TimeProvider.Testing` (FakeTimeProvider) · `ReScene.App.Core` + `ReScene.Lib` submodule. Spec: `docs/superpowers/specs/2026-07-11-reconstruction-fixes-design.md` (rev. 7). This plan was refined across six codex reviews. Rev. 7 is **frozen for execution**: it keeps every in-scope fix (transactional placement, expected-identity completeness, plan-before-mutate, reparse-safe canonical sources, no-file-list self-copy rejection, scan-safe version capture, exact-root path semantics) and **deliberately excludes two out-of-scope items** the reviewer pushed that fall outside the 25 findings and the settled lib boundary — see the Scope-boundary constraint. Grounded in a read of the engine (`Manager`/`SRRReconstructor`/`InputDirectoryPreparer`), the `-mt`/switch matrix builder, and the version-selection code (`RARVersionSelector`).
...
- `-mt` matrix (`RARCommandLineBuilder.cs:267-268,354`): today `mtLo = SwitchMT ? Math.Max(1, …) : 0` **drops `-mt0`**, and each arg is `new($"-mt{z}", 360)` (Min 360, **no Max**). `#13` (in scope) fixes only: preserve `-mt0`, clamp both endpoints to `0..64`, build off the UI thread, and cap cardinality. (Note for a future finding, **out of scope here**: `-mt` capability is really per *executable version* — `<360` none, `360-499` max 16, `>=500` max 64 — and `FilterArgumentsForVersion` (`RARVersionSelector.cs:117-122`) drops an inapplicable arg rather than skipping the row, so a high `-mt` on a low exe degrades to a duplicate no-`-mt` command; this pre-existing behavior is left unchanged.)
...
- **Scope boundary:** lib changes are limited to `#7` (per-set directories), `#9` (canonical dir-qualified CRC), and the architecture-approved committed-files result (`#3/#4/#5`, D1 — required by the approved uniform scratch-then-relocate design). **Explicitly out of scope** (a candidate future finding, NOT `#13`): making `-mt` correct per *executable version* (the pre-existing degraded-duplicate behavior when a high `-mt` is passed to a low exe). `#13` here is only its filed scope — UI thread, overflow, clamp, `-mt0`, cardinality cap. Also out of scope: a unified `ReconstructionRunPlan`/progress-accounting refactor (the plan-before-mutate reorder is surgical, not a new abstraction).
...
- [ ] **Step 2 — run FAIL.**
- [ ] **Step 3 — implement:** add `LastSegment`; qualified-first + basename fallback keyed on ONE canonical key; replace `Path.GetFileName(originalNames[i/0])` with `LastSegment`.
...
- **Completeness is against the expected volume identity for the mode, not "what was found":** the expected set is derived from `RAROptions.OriginalRARFileNames` (the release volume names — not `expectedInOrder`, which deliberately omits CRC-less volumes, `Manager.cs:147`). For **non-CAV** (`CompleteAllVolumes==false`) the expected set is the **single first** volume; for **CAV and custom** it is **all** volumes. `Complete` requires the placed set to match that expected identity by **count and normalized name** — unconditionally (not gated on `RenameToReleaseNames`; generated-name output is validated logically by carrying `(ExpectedName, Path)` per committed file). A run that placed fewer than the mode's expected volumes is **not** complete.
...
- **Grouped-match contract:** exploratory mode (`StopOnFirstMatch==false`) continues across **executable versions** and `TryProcessCommandLinesAsync` returns on the **first** matching command per version (`Manager.cs:821`), so `Matches` holds **at most one committed match per executable version**, in version order, `Matches[0]` first-kept.
- **Custom packer completeness:** `SRRReconstructor` must require the **full expected volume set** before reporting success — today it returns success on `allMatched && completedVolumes > 0` (`SRRReconstructor.cs:345`), which passes when only some expected volumes were produced. Require `completedVolumes == expectedVolumeCount` (compared by normalized name + count) regardless of `CompleteAllVolumes`/rename flags; `CustomPackerFiles` lists that full set.
...
- [ ] **Step 1 — failing tests (pure/seam where possible):**
  ...
  - exploratory outer loop with **two matching executable versions** and `StopOnFirstMatch==false` → `Matches.Count==2` (one per version, in version order), `Matches[0]` is the FIRST version's match, `result.Combo==Matches[0].Combo` (first-kept, not last-overwritten).
...
**Scope of #13 (as filed):** the finding is "`-mt` matrix built on the UI thread, unclamped, overflow-unsafe." This task fixes exactly that — off-thread, both endpoints clamped, `-mt0` preserved, a checked cardinality cap. It does **not** change per-executable `-mt` validity: `-mt` stays a uniform `new($"-mt{z}", 360)` arg (unchanged from today). The degraded-duplicate behavior for high `-mt` on low exes is **pre-existing** and out of `#13`'s scope — see the Scope-boundary note in Global Constraints.

**Files:** `RARCommandLineBuilder.cs` (267-268 range normalization + clamp, build entry → token overload + cardinality cap), `ReconstructorViewModel.cs` (`SwitchMTStart/End` setters 714-715 clamp to 0..64; global-matrix call site off-thread); Test `RARCommandLineBuilderTests`.
...
- (b) **clamps BOTH endpoints to `0..64` BEFORE ordering** — `lo = min(clamp(Start), clamp(End))`, `hi = max(…)` — so `100..200` clamps to `64..64` (single `-mt64`), never an empty loop (64 is the max threads any WinRAR accepts).
- (c) keeps each `-mt{z}` as `new($"-mt{z}", 360)` (unchanged).
...
- [ ] **Step 1 — failing tests:** `SwitchMTEnd=int.MaxValue`→clamped to 64, no overflow; `-mt0` preserved when `Start==0`; **`Start=100,End=200` → single `-mt64` row, not empty**; a cap-exceeding matrix→typed exception before allocation; a cancelled token→`OperationCanceledException` promptly.
- [ ] **Step 2 — run FAIL.**
- [ ] **Step 3 — implement:** allow 0; clamp both endpoints to 0..64 before ordering; keep `-mt{z}` as `new($"-mt{z}", 360)`; checked cardinality + cap; periodic `ct.ThrowIfCancellationRequested()`; clamp the VM setters to 0..64; the global-matrix call site runs off the UI thread.
- [ ] **Step 4 — run PASS; commit** (`fix: bounded, cancellable matrix builder, -mt0 preserved (#13)`).
...
- [ ] **Step 2 — run FAIL.**
- [ ] **Step 3 — implement:** replace lexical compare with `ReconstructionPathGuard.ResolveReal` + filesystem-correct comparer; flag verification/release/imported paths that `Overlaps` the resolved `output` or `.rescene-work` roots — same-as, under, or above (not the bare `OutputPath`; use `IsSameOrDescendant`/`Overlaps`, so an exact-root match is caught, not just strict descendants); fail closed on indeterminate.
...
**Plan before mutate:** set resolution (`ResolveSets`) and all reject-the-run decisions (multi-set custom-packer, reserved-root distinctness, live-input overlap, and — when the SRR has no archive file list — release/`OutputPath` overlap) happen **before** the destructive pre-run cleanup **and before any destructive confirm dialog** — including the wizard's own delete-confirmation (`BeginnerWizardFactory.cs:190`), which is gated on the preflight passing. Today cleanup runs at `ReconstructorViewModel.cs:1417` while sets are only resolved inside `RunArchiveSetsAsync` (`:1557`), so an already-known-unsupported run would still erase existing output. Move planning ahead of mutation (a surgical reorder — a reject predicate that runs before cleanup/confirm — not a new run-plan abstraction).
...
- [ ] **Step 3 — implement:**
  ...
  - **Branch-specific source guard:** brute-force → each `Matches[0].Files` path `ResolveReal`s to an existing, unique regular file whose **final component is not a reparse point**, canonically contained strictly under **`<workRoot>\output`**; custom → each `CustomPackerFiles` path canonically under the **custom work root** (`<workRoot>`). **Canonicalize before the uniqueness check and move the canonical resolved path.** Reject otherwise (set fails, no partial move).
