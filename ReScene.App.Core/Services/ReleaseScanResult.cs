...
/// split into the six destinations <c>generate_srr</c> ultimately files a
/// release's files into. Main-set classification populates
/// <see cref="MainSets"/>, <see cref="SubtitleSfvs"/>, the proof share of <see cref="StoredFiles"/>,
/// and <see cref="Warnings"/>; <see cref="SampleFiles"/> and <see cref="MusicSfvs"/> come from
/// sample detection and rescue-fallback music re-admission.
/// </summary>
/// <param name="MainSets">The release's main RAR sets, in traversal order.</param>
/// <param name="SampleFiles">Sample media files.</param>
...
/// subpack/subfix release.
/// </param>
/// <param name="StoredFiles">
/// Files whose raw bytes get embedded verbatim in the SRR — proof SFV/RAR pairs from rule 4;
/// NFOs, logs, cues, etc. from stored-file collection.
/// </param>
/// <param name="MusicSfvs">
/// SFVs detected as music sets — only reachable via the rescue fallback
/// (<c>[DIVERGENCE]</c> pyrescene admits them as ordinary main sets; this port routes them here
/// instead per Spec 2's music-set handling). Otherwise populated during music detection.
/// </param>
/// <param name="Warnings">
