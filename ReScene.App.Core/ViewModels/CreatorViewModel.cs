...
public partial class CreatorViewModel : OperationViewModelBase
{
    ...
    public CreatorViewModel(ISRRCreationService srrService, ISRSCreationService srsService, IFileDialogService fileDialog, ITempDirectoryService tempDir, IAppSettingsService settingsService, IUiDispatcher uiDispatcher, IReleaseScanner releaseScanner, Func<string>? workDirFactory = null)
    {
        ...
        // Task 9 (spec §3 "Generated artifacts"): a unique working directory per Create run, used
        // ONLY by folder mode's generated-artifact staging (StageFolderArtifactsAsync) — separate
        // from the pre-existing ITempDirectoryService (_tempDir), which the file-mode/wizard
        // placeholder path already owns. Injectable so a test can observe/assert on the exact path.
    ...
    // be built from a scan that never actually looked at the release (F3).
    ...
    partial void OnInputPathChanged(string value)
    {
        ...
        // either mode) — spec §3 "Output path" applies uniformly to both input kinds, not just Browse.
    ...
    private async Task CreateSRRAsync()
    {
        ...
        try
        {
            ...
            if (_isFolderMode)
            {
                // Folder mode (spec §3): stage generated artifacts (SRS/nested SRR/failure txt,
                // Task 9) into the stored-file list before handing everything to the multi-input
                // writer.
        ...
        catch (OperationCanceledException)
        {
            ...
            //
            // E5 re-confirmed (codex #5, second round): the OCE is intentionally NOT rethrown —
            // the command's own Task completes normally (RanToCompletion, not Canceled), with the
            // cancellation instead made observable through this VM's own state (ProgressMessage,
            // BuildSucceeded staying false, the "Cancelled" log entry) and IsCreating resetting to
            // false in `finally` below so the command becomes runnable again. This matches every
            // OTHER cancellable command in the codebase — ReconstructorViewModel's
            // ExecuteReconstructionAsync and FileCompareViewModel's diff both swallow their own OCE
            // the same way — so rethrowing here would be the outlier, not the fix.
    ...
    /// null'd out by an older scan's cleanup (F1).
    /// </summary>
    ...
    private void StartFolderScan(string releaseRoot)
    {
        ...
        if (IsFilesystemRoot(releaseRoot))
        {
            ...
            // input that was rejected outright (F3). Reset music-only too: a prior scan's gate must
            // not linger once the input itself becomes invalid.
            IsScanning = false;
            _isMusicOnlyFolder = false;
        ...
        // exact `cts` first (F1, discovered via RapidInputSwitching_WithoutAwaiting_NeverThrows).
    ...
    private async Task RunFolderScanAsync(string releaseRoot, int generation, CancellationTokenSource cts, CancellationToken token)
    {
        try
        {
            ...
            _uiDispatcher.Post(() =>
            {
                // F1: every _scanCts read/write below happens inside this Post callback — which,
                // like every other UI-thread-invoked entry point (a property-changed hook, Reset),
                // is serialized onto the UI thread — never on the background thread that ran the
    ...
    private void ApplyFolderScanResult(string releaseRoot, ReleaseScanResult result)
    {
        ...
        if (IsRootError(result))
        {
            ...
            // let Create build an empty/header-only SRR from a root that was never actually read (F3).
            _isMusicOnlyFolder = false;
            _folderScanInvalid = true;
            ClearFolderScanResults();
        ...
        // A successful scan clears any earlier invalid/error state (F3c) — Create is re-enabled
        // once the input points at something the scanner could actually read.
        _folderScanInvalid = false;
    ...
    /// or itself a still-current auto value, never a user-typed or user-picked one (spec §3 "Output
    /// path", finding 15). Shared by both input kinds so switching between them consistently
    /// replaces an auto value and preserves a genuine user edit either way (F2). Returns whether it
    /// applied the auto value (vs. leaving a user-owned one untouched), for callers that only want
    /// to surface UI feedback (e.g. an Info status) when an auto-fill actually happened.
    /// </summary>
    ...
    /// <summary>
    /// Detects a scanner root-enumeration failure (design spec §2 "Error contract" —
    /// <see cref="ReleaseScanResult.RootError"/>: every collection empty except a single warning in
    /// that method's exact "Cannot scan '&lt;root&gt;': ..." format). <see cref="ReleaseScanResult"/>
    /// carries no explicit RootFailed flag, so this is distinguished from an ordinary empty scan
    ...
    /// </summary>
    private static bool IsRootError(ReleaseScanResult result) =>
        ...
        List<StoredFileEntry> baseline, string workDir, SRRCreationOptions options, CancellationToken ct)
    {
        ...
        // produced — no collision error, the generated one simply replaces it.
    ...
    /// <summary>
    /// The position where generated SAMPLE artifacts belong (excerpt's own SRS pass, L646-772):
    /// immediately before whichever comes first — a pre-existing <c>.srs</c> (any path); ANY
    /// <c>.sfv</c> (main, non-main, or proof-linked — every sfv collectively marks the START of
    /// pass-10's tail, so samples must precede all of them, not just non-proof ones); or a
    ...
    /// </summary>
    private static int FindSampleArtifactSpliceIndex(List<StoredFileEntry> entries)
    {
    ...
    /// but BEFORE the final input-SFV pass, L803-839/L1190-1206): the first <c>.sfv</c> entry of
    /// ANY kind — main, non-main, or proof-linked — always marks the START of pass-10's tail
    /// (every sfv collectively lives there), naturally skipping past a fix RAR (<c>.rar</c>, not
    /// <c>.sfv</c>) that might precede it.
    /// </summary>
    private static int FindSubtitleArtifactSpliceIndex(List<StoredFileEntry> entries)
    {
        for (int i = 0; i < entries.Count; i++)
        {
    ...
    /// <summary>
    /// E2 fix (codex #2, second round): whether a stored logical name (forward-slash-separated)
    /// sits under a <c>proof</c>/<c>proofs</c> DIRECTORY — the IMMEDIATE parent only, matching the
    /// scanner's rule-3/4 pardir classification exactly (ReleaseScanner.cs, excerpt L342/L357).
    /// The first round's version accepted ANY ancestor segment, broader than the scanner's own
    /// immediate-parent test.
    /// </summary>
    ...
    /// <summary>
    /// E2 fix (codex #2, second round) — "reconcile the splice anchor with the post-reorder
    /// final-SFV region": mirrors <see cref="ReleaseScanner.ApplyProofBeforeSfvReorder{T}"/>'s own
    /// mover predicate (stem, with its last 4 characters swapped to <c>.sfv</c>, matching some
    /// OTHER entry). A proof RAR satisfying this has either already been relocated next to its sfv
    ...
    /// </summary>
    private static bool HasMatchingSfv(string storedName, List<StoredFileEntry> entries)
    {
    ...
    private async Task<List<StoredFileEntry>> GenerateSampleArtifactsAsync(string workDir, SRRCreationOptions options, CancellationToken ct)
    {
        ...
        for (int i = 0; i < samples.Count; i++)
        {
            ...
            // excerpt L743-771 — a RAR-backed .vob sample keeps its SRS AND gets a nested SRR
            // (both, not a replacement). Only attempted once the SRS itself exists (a [DIVERGENCE:
            ...
            if (!IsRarBackedVobSample(sample))
            {
                continue;
            }
    ...
    /// <summary>
    /// excerpt L744-751 — a RAR-backed sample is a lowercase-<c>.vob</c> file (case-SENSITIVE: a
    ...
    /// </summary>
    private static bool IsRarBackedVobSample(string samplePath)
    {
    ...
    /// command, whose source never reaches the scanner's traversal at all (D4 was over-broad: it
    /// assumed pass-10 always covers it, which is false for a manually-added one) — those need
    /// storing here. Both cases are reconciled with one OS-final-path dedup check (matching the
    /// scanner's own <see cref="ReleaseScanner.ResolveDedupKey"/> dedup discipline) instead of two
        ...
        List<StoredFileEntry> currentStored, string workDir, SRRCreationOptions options, bool generateNestedSrrs, CancellationToken ct)
    {
        ...
        for (int i = 0; generateNestedSrrs && i < subtitleSfvs.Count; i++)
        {
            ...
            // E4 fix (codex #4, second round — spec §3 L233-234, REVERSING the first round's D5
            // adjudication, which wrongly treated this as dead/out-of-scope): the "directory"
            // half of `stem` (everything up to the SFV's own last path segment) is reused for
            // EVERY nested SRR this subtitle SFV produces — each chain keeps ITS OWN first-RAR
            // basename as the file-name half (create_srr_for_subs, excerpt L1072-1204), not the
            // SFV's.
    ...
    /// <summary>
    /// E4 fix (codex #4, second round): one nested SRR PER RAR CHAIN the subtitle SFV lists —
        ...
        string sfvPath, string dirPrefix, string workDir, int index, SRRCreationOptions options, CancellationToken ct)
    {
        ...
        // E6 fix (codex #6, second round — excerpt L1489): pyrescene's create_srr_for_subs
        // HARDCODES its own nested-creation call's options (`save_paths=False, compressed=True,
        // oso_hash=False`) rather than forwarding whatever the OUTER SRR's options happen to be —
        // so a user enabling ComputeOSOHashes for the outer SRR must not leak OSO blocks into a
        // nested subtitle SRR pyrescene never adds them to. `save_paths=False` needs no explicit
        // handling: CreateFromRARAsync (unlike CreateFromInputsAsync) has no relative-path mode at
        // all, so it is already flat-name-only by construction.
        var nestedOptions = new SRRCreationOptions
        {
            AppName = options.AppName,
            AllowCompressed = true,
        ...
        {
            ct.ThrowIfCancellationRequested();
            // Task 9 fix-4 / F1: SfvVolumeResolver now returns each chain in first-seen LISTING
            // order and no longer sorts (so the writer's single sort stays byte-identical to base —
            // see SfvVolumeResolver.ResolveOrderedChains). This path, unlike the writer, needs the
            // TRUE first volume (volume[0]) for chainStem/naming and passes the volumes to
    ...
    private async Task CreateVobsubSRRsAsync(string releaseDir, SRRCreationOptions options, string tempDir, CancellationToken ct)
    {
        ...
        // E4+E6 (Task 9 fix-4, user-approved): the Advanced-tab create-time scan now produces one
        // nested SRR PER RAR CHAIN by REUSING the folder-mode GenerateNestedSubtitleSrrsAsync — which
        // already does the chain-split AND builds its own nestedOptions (ComputeOSOHashes=false,
        // AllowCompressed=true), so a multi-language subtitle SFV yields per-chain SRRs (never one
        // merged SRR) and a user enabling ComputeOSOHashes for the outer run cannot leak OSO blocks
        // into them. GenerateAndRecordAsync's 1-source->1-path shape no longer fits (a single SFV can
        // now yield multiple SRRs), so record the returned list directly; the per-source index still
        ...
        for (int i = 0; i < subtitleSfvs.Count; i++)
        {
            ...
            // dirPrefix = the directory half of the Advanced-tab GeneratedStoredName("Subs")
            ...
            string generatedName = GeneratedStoredName(releaseDir, sfv, ".srr", "Subs");
    ...
    ///
    /// [DIVERGENCE: deferred to Task 10] Unlike the Advanced-tab create-time path
    /// (<see cref="CreateVobsubSRRsAsync"/>, which now emits one nested SRR PER RAR CHAIN via
    ...
    /// multiple chains still produces ONE merged SRR here. E4 (per-chain) support needs the Task-10
    /// wizard model that can materialize one placeholder into several stored items. E6 (oso-off) IS
    ...
    private async Task<string?> GenerateNestedSRRFileAsync(string sfvPath, string tempDir, int index, SRRCreationOptions options, CancellationToken ct)
    {
        ...
        // E6 (Task 9 fix-4, excerpt L1489): force the nested subtitle SRR's own options
        // (AllowCompressed=true, ComputeOSOHashes=false) — mirroring the folder-mode nestedOptions in
