...
/// <summary>
/// Assembles the wizard (navigation VM + body view) for a Beginner hub card. Avalonia port of the WPF
/// <c>ReScene.NET.Views.Wizards.BeginnerWizardFactory</c>: identical step lists, per-VM resets, and
/// pre-sets. The only behavioural difference is the confirm dialogs — WPF's
/// <c>MessageBox.Show(..., OKCancel, Warning)</c> is replaced by the synchronous
/// <c>shell.FileDialog.Confirm(title, message)</c> (the Avalonia impl runs a sync modal), so the
/// wizard's ConfirmLeave gates behave exactly as before.
/// </summary>
public static class BeginnerWizardFactory
{
    public static (WizardViewModel ViewModel, Control Body) Create(BeginnerCard card, BeginnerShellViewModel shell)
    {
        ...
        switch (card)
        {
            case BeginnerCard.CreateSRR:
                ...
                return BuildCreateSRR(shell.CreateSRRWizard, shell);
            case BeginnerCard.CreateSRS:
                shell.SRSCreator.Reset();
                return BuildCreateSRS(shell.SRSCreator, shell);
            case BeginnerCard.Reconstruct:
                shell.Reconstructor.Reset();
                return BuildReconstruct(shell.Reconstructor, shell);
            case BeginnerCard.Restore:
                shell.Restore.Reset();
                return BuildRestore(shell.Restore);
            case BeginnerCard.EditSRR:
                shell.SRREditor.Reset();
                return BuildEditSRR(shell.SRREditor, shell);
            default:
                throw new ArgumentOutOfRangeException(nameof(card));
        }
    }

    private static (WizardViewModel, Control) BuildCreateSRR(CreatorViewModel vm, BeginnerShellViewModel shell)
    {
    ...
    private static (WizardViewModel, Control) BuildCreateSRS(SRSCreatorViewModel vm, BeginnerShellViewModel shell)
    {
        var steps = new List<WizardStep>
        {
            ...
            new()
            {
                ...
                ConfirmLeave = () =>
                {
                    if (!vm.HasValidMainFile && !shell.FileDialog.Confirm(
                    ...
                    if (File.Exists(vm.OutputPath) && !shell.FileDialog.Confirm(
    ...
    private static (WizardViewModel, Control) BuildReconstruct(ReconstructorViewModel vm, BeginnerShellViewModel shell)
    {
    ...
    private static (WizardViewModel, Control) BuildRestore(BeginnerRestoreViewModel vm)
    {
        var steps = new List<WizardStep>
        {
    ...
    private static (WizardViewModel, Control) BuildEditSRR(SRREditorViewModel vm, BeginnerShellViewModel shell)
    {
        var steps = new List<WizardStep>
        {
            ...
            new()
            {
                ...
                ConfirmLeave = () =>
                {
                    ...
                    return shell.FileDialog.Confirm(
                        "Overwrite existing file?",
                        $"A file already exists at:\n\n{vm.OutputPath}\n\nDo you want to overwrite it?");
                },
                OnLeave = vm.Save,
